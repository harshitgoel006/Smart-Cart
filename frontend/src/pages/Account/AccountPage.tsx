...
export function AccountPage() {
  ...
  const updateProfile = async (event: FormEvent) => { event.preventDefault(); startAction(); const emailChanged = profile.email.trim().toLowerCase() !== user.email.toLowerCase(); try { await sendJson('/users/update-account', 'PATCH', profile); await refreshUser(); setEmailVerificationPending(emailChanged); setMessage(emailChanged ? 'Profile saved. Check your new email for the verification code.' : 'Profile details saved.') } catch (reason) { setError((reason as Error).message) } finally { setBusy(false) } }
  const verifyEmail = async (event: FormEvent) => { event.preventDefault(); startAction(); try { await sendJson('/users/verify-email-change', 'POST', { email: profile.email, otp: emailOtp }); await refreshUser(); setEmailVerificationPending(false); setEmailOtp(''); setMessage('Email verified successfully.') } catch (reason) { setError((reason as Error).message) } finally { setBusy(false) } }
  ...
  const updateProfileField = (key: keyof typeof profile, value: string) => setProfile((current) => ({ ...current, [key]: value }))
