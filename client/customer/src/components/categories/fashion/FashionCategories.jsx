import { Link } from "react-router-dom";
import { motion } from "framer-motion";
import { FiArrowUpRight } from "react-icons/fi";

// A visually stunning and immersive fashion categories section that captures the essence of style and sophistication. The design features a clean and modern layout with a focus on high-quality imagery, bold typography, and subtle animations to create an engaging user experience. Each category is presented as a large, clickable card with a captivating image, a stylish overlay, and a clear call-to-action that invites users to explore the collection. The use of soft gradients, elegant fonts, and dynamic hover effects adds a touch of luxury and exclusivity to the overall design, making it an irresistible gateway to the world of fashion.
const categories = [
  {
    name: "Women's Couture",
    slug: "women-wear",
    tag: "Dresses, Tops & Sets",
    image:
      "https://i.pinimg.com/736x/bf/ef/c0/bfefc07ae2e252a392463a0e54057c49.jpg",
  },
  {
    name: "Men's Essentials",
    slug: "men-wear",
    tag: "Shirts, Polos & Denim",
    image:
      "https://i.pinimg.com/1200x/bd/b3/28/bdb328662efd4472f5ad1f080e0ee578.jpg",
  },
  {
    name: "Junior Styles",
    slug: "kids-wear",
    tag: "Playful Prints & Comfort",
    image:
      "https://i.pinimg.com/1200x/c8/fa/17/c8fa1761ad6aff8c7c9cd345e5f9aa38.jpg",
  },
  {
    name: "Footwear Hub",
    slug: "footwear",
    tag: "Heels, Sneakers & Formals",
    image:
      "https://i.pinimg.com/736x/42/e7/58/42e758a02a13e18ee2f7c3129af4e12a.jpg",
  },
  {
    name: "Accessories",
    slug: "accessories",
    tag: "Bags, Watches & Jewelry",
    image:
      "https://i.pinimg.com/736x/8e/11/36/8e113664f3db1a1cec60bdba483ec6c1.jpg",
  },
  {
    name: "Occasion Wear",
    slug: "festive",
    tag: "Ethnic & Party Ready",
    image:
      "https://i.pinimg.com/1200x/5e/1a/91/5e1a913d16146b1d9311432c007b7a64.jpg",
  },
];

const FashionCategories = () => {
  return (
    <section className="relative py-20 bg-[#fdfdfd] overflow-hidden">
      
      {/* LIGHT AMBIENT BACKGROUND */}
      <div className="absolute inset-0 pointer-events-none">
        <motion.div 
          animate={{ x: [0, 40, 0], y: [0, 20, 0] }}
          transition={{ duration: 22, repeat: Infinity }}
          className="absolute -top-[15%] -left-[10%] w-[45%] h-[45%] bg-rose-300/30 rounded-full blur-[140px]" 
        />
        <motion.div 
          animate={{ x: [0, -30, 0], y: [0, -30, 0] }}
          transition={{ duration: 18, repeat: Infinity }}
          className="absolute top-[25%] -right-[10%] w-[40%] h-[40%] bg-indigo-300/25 rounded-full blur-[140px]" 
        />
      </div>

      <div className="max-w-[1440px] mx-auto px-8 relative z-10">
        
        {/* HEADER */}
        <div className="mb-16 space-y-4">
          <motion.div
            initial={{ opacity: 0, x: -20 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            className="flex items-center gap-4"
          >
            <span className="h-[2px] w-12 bg-gray-900" />
            <span className="text-xs font-black tracking-[0.4em] uppercase text-gray-400">
              Curated Style Labs
            </span>
          </motion.div>
          
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <motion.h2
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8 }}
              viewport={{ once: true }}
              className="text-4xl md:text-5xl font-black tracking-tighter text-gray-900 leading-[0.9]"
            >
              FASHION <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-rose-500 via-purple-500 to-indigo-400">
                UNIVERSE
              </span>
            </motion.h2>
            
            <motion.p 
              initial={{ opacity: 0 }}
              whileInView={{ opacity: 1 }}
              transition={{ delay: 0.4 }}
              viewport={{ once: true }}
              className="max-w-xs text-sm font-bold text-gray-400 uppercase tracking-widest leading-relaxed"
            >
              Style for every generation. Every story. Every you.
            </motion.p>
          </div>
        </div>

        {/* COMPACT GRID */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {categories.map((cat, i) => (
            <motion.div
              key={cat.slug}
              initial={{ opacity: 0, y: 40 }}
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ delay: i * 0.08, duration: 0.7 }}
              viewport={{ once: true }}
              whileHover={{ y: -8 }}
              className="group"
            >
              <Link
                to={`/products?sub=${cat.slug}`}
                className="
                  relative block h-[260px] md:h-[300px] overflow-hidden rounded-[2.5rem]
                  bg-white transition-all duration-700
                  shadow-[0_15px_40px_-15px_rgba(0,0,0,0.12)]
                  hover:shadow-[0_30px_70px_-20px_rgba(244,63,94,0.3)]
                "
              >
                {/* IMAGE */}
                <motion.img
                  src={cat.image}
                  alt={cat.name}
                  whileHover={{ scale: 1.1 }}
                  transition={{ duration: 1.1, ease: [0.33, 1, 0.68, 1] }}
                  className="h-full w-full object-cover grayscale-[15%] group-hover:grayscale-0"
                />

                {/* SOFT OVERLAY */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-75 group-hover:opacity-90 transition-opacity" />

                {/* CONTENT */}
                <div className="absolute inset-0 p-7 flex flex-col justify-end">
                  <span className="text-[9px] font-black tracking-[0.3em] uppercase text-rose-300 mb-1">
                    {cat.tag}
                  </span>

                  <div className="flex items-center justify-between">
                    <h3 className="text-2xl font-black text-white tracking-tight uppercase italic">
                      {cat.name}
                    </h3>

                    <div className="
                      h-10 w-10 rounded-full bg-white text-black
                      flex items-center justify-center
                      shadow-xl scale-0 group-hover:scale-100
                      transition-all duration-500
                    ">
                      <FiArrowUpRight size={20} />
                    </div>
                  </div>
                </div>

                {/* ACCENT LINE */}
                <div className="absolute bottom-0 left-0 h-1 w-0 bg-gradient-to-r from-rose-500 to-indigo-500 group-hover:w-full transition-all duration-700" />
              </Link>
            </motion.div>
          ))}
        </div>
      </div>
    </section>
  );
};

export default FashionCategories;